	...
	<view class="hot-page-container">
		...
		@refresherrefresh="onRefresh">
			...
			<view class="hot-page-wrap" v-for="(item,index) in hotListStore" :key="item.id">
				...
				<view class="hot-post-body-text-imgnull" v-if="item.imgIsNull" @click="toInformationDetail(item,index)">
					{{ item.textContent }}
				</view>
				<view class="hot-post-body-info">
					...
					<view class="post-body-info-item ">
						<view class="cuIcon-like" style="color: #fbbd08;font-size:1.5em" v-if="item.hasLoved == 0"
				...
				<view class="hot-post-body-text" v-if="!item.imgIsNull" @click="toInformationDetail(item,index)">
					{{ item.textContent }}
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			...mapMutations(['changeHotListStore', 'hotPageNext', 'hotPageInit','loveInforHotStore','unloveInforHotStore']),
			...
			updataHotPage() {
				...
				this.hotPageNext();
				const {num,size} = this.hotListPage;
